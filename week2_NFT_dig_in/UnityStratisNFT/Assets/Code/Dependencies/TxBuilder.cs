...
using Builder = System.Func<NBitcoin.TransactionBuilderSC.TransactionBuildingContext, NBitcoin.IMoney>;
...
namespace NBitcoin
{
    ...
    /// </summary>
    public class TransactionBuilderSC
    {
        internal class TransactionBuilderSigner : ISigner
        {
            ...
            private TransactionBuilderSC _builderSc;
            public TransactionBuilderSigner(TransactionBuilderSC builderSc, ICoin coin, SigHash sigHash, IndexedTxIn txIn)
            {
                this._builderSc = builderSc;
                this.coin = coin;
            ...
            {
                return this.txIn.Sign(this._builderSc.Network, key, this.coin, this.sigHash);
            }
        ...
        internal class TransactionBuilderKeyRepository : IKeyRepository
        {
            ...
            private TransactionBuilderSC _txBuilderSc;
            public TransactionBuilderKeyRepository(TransactionBuilderSC txBuilderSc, TransactionSigningContext ctx)
            {
                this._Ctx = ctx;
                this._txBuilderSc = txBuilderSc;
            }
            #region IKeyRepository Members

            public Key FindKey(Script scriptPubkey)
            {
                return this._txBuilderSc.FindKey(this._Ctx, scriptPubkey);
            }
        ...
        private class KnownSignatureSigner : ISigner, IKeyRepository
        {
            ...
            public KnownSignatureSigner(TransactionBuilderSC builderSc, List<Tuple<PubKey, ECDSASignature>> _KnownSignatures, ICoin coin, SigHash sigHash, IndexedTxIn txIn)
            {
                this._builderSc = builderSc;
            ...
            public Key FindKey(Script scriptPubKey)
            {
                ...
                {
                    uint256 hash = this.txIn.GetSignatureHash(this._builderSc.Network, this.coin, this.sigHash);
        ...
        {
            public TransactionSigningContext(TransactionBuilderSC builderSc, Transaction transaction)
            {
            ...
            public TransactionBuilderSC BuilderSc
            {
                get;
                set;
            }
        ...
        {
            public TransactionBuildingContext(TransactionBuilderSC builderSc)
            {
                this.BuilderSc = builderSc;
                this.Transaction = builderSc.Network.CreateTransaction();
            ...
            {
                var memento = new TransactionBuildingContext(this.BuilderSc);
            ...
            public void RestoreMemento(TransactionBuildingContext memento)
            {
                ...
                this.Transaction = memento.BuilderSc.Network.CreateTransaction(memento.Transaction.ToBytes());
        ...
        {
            private TransactionBuilderSC _Parent;
            public BuilderGroup(TransactionBuilderSC parent)
            {
                this._Parent = parent;
                this.FeeWeight = 1.0m;
        ...
        public TransactionBuilderSC(Network network)
        {
        ...
        public TransactionBuilderSC(int seed, Network network)
        {
            this.Network = network;
        ...
        public TransactionBuilderSC SetLockTime(LockTime lockTime)
        {
        ...
        public TransactionBuilderSC SetTimeStamp(uint timeStamp)
        {
        ...
        public TransactionBuilderSC AddKeys(params ISecret[] keys)
        {
            AddKeys(keys.Select(k => k.PrivateKey).ToArray());
            return this;
        }

        public TransactionBuilderSC AddKeys(params Key[] keys)
        {
        ...
        public TransactionBuilderSC AddKnownSignature(PubKey pubKey, TransactionSignature signature)
        {
            if (pubKey == null)
        ...
        public TransactionBuilderSC AddKnownSignature(PubKey pubKey, ECDSASignature signature)
        {
            if (pubKey == null)
                throw new ArgumentNullException("pubKey");
            if (signature == null)
        ...
        public TransactionBuilderSC AddCoins(params ICoin[] coins)
        {
            return AddCoins((IEnumerable<ICoin>)coins);
        }

        public TransactionBuilderSC AddCoins(IEnumerable<ICoin> coins)
        {
        ...
        /// <returns></returns>
        public TransactionBuilderSC SetGroupName(string groupName)
        {
        ...
        /// <returns></returns>
        public TransactionBuilderSC Send(IDestination destination, Money amount)
        {
        ...
        /// <param name="amount">The amount</param>
        /// <returns></returns>
        public TransactionBuilderSC Send(Script scriptPubKey, Money amount, bool ignoreFees = false)
        {
            ...
            if (!ignoreFees && this.DustPrevention && amount < GetDust(scriptPubKey) && !_OpReturnTemplate.CheckScriptPubKey(scriptPubKey))
            {
                SendFees(amount);
                return this;
            }
        ...
        /// </summary>
        /// <returns></returns>
        public TransactionBuilderSC SubtractFees()
        {
        ...
        public TransactionBuilderSC Send(IDestination destination, IMoney amount)
        {
            return Send(destination.ScriptPubKey, amount);
        }
        ...
        public TransactionBuilderSC Send(Script scriptPubKey, IMoney amount)
        {
        ...
        /// <returns></returns>
        public TransactionBuilderSC SendAsset(IDestination destination, AssetMoney asset)
        {
        ...
        /// <param name="asset">The asset and amount</param>
        /// <returns></returns>
        public TransactionBuilderSC SendAsset(IDestination destination, AssetId assetId, ulong quantity)
        {
            return SendAsset(destination, new AssetMoney(assetId, quantity));
        }

        public TransactionBuilderSC Shuffle()
        {
        ...
        public TransactionBuilderSC SendAsset(Script scriptPubKey, AssetId assetId, ulong assetQuantity)
        {
            return SendAsset(scriptPubKey, new AssetMoney(assetId, assetQuantity));
        }

        public TransactionBuilderSC SendAsset(Script scriptPubKey, AssetMoney asset)
        {
            if (asset.Quantity < 0)
        ...
        public TransactionBuilderSC SetTransactionPolicy(StandardTransactionPolicy policy)
        {
        ...
        public TransactionBuilderSC Send(BitcoinStealthAddress address, Money amount, Key ephemKey = null)
        {
            if (amount < Money.Zero)
                throw new ArgumentOutOfRangeException("amount", "amount can't be negative");
        ...
        public TransactionBuilderSC IssueAsset(IDestination destination, AssetMoney asset)
        {
        ...
        public TransactionBuilderSC IssueAsset(Script scriptPubKey, AssetMoney asset)
        {
            AssertOpReturn("Colored Coin");
        ...
        public TransactionBuilderSC SendFees(Money fees)
        {
            if (fees == null)
        ...
        /// <returns></returns>
        public TransactionBuilderSC SendEstimatedFees(FeeRate feeRate)
        {
        ...
        /// <param name="feeRate"></param>
        /// <returns></returns>
        public TransactionBuilderSC SendEstimatedFeesSplit(FeeRate feeRate)
        {
            this.FilterUneconomicalCoinsRate = feeRate;
            Money fee = EstimateFees(feeRate);
            SendFeesSplit(fee);
        ...
        /// <returns></returns>
        public TransactionBuilderSC SendFeesSplit(Money fees)
        {
            if (fees == null)
                throw new ArgumentNullException("fees");
        ...
        /// <returns></returns>
        public TransactionBuilderSC SetFeeWeight(decimal feeWeight)
        {
            this.CurrentGroup.FeeWeight = feeWeight;
            return this;
        }

        public TransactionBuilderSC SetChange(IDestination destination, ChangeType changeType = ChangeType.All)
        {
            return SetChange(destination.ScriptPubKey, changeType);
        }

        public TransactionBuilderSC SetChange(Script scriptPubKey, ChangeType changeType = ChangeType.All)
        {
        ...
        public TransactionBuilderSC SetCoinSelector(ICoinSelector selector)
        {
            if (selector == null)
        ...
        public ICoin FindSignableCoin(IndexedTxIn txIn)
        {
            ...
            if (hash != null)
            {
                ...
                if (redeem != null)
                    return new ScriptCoinSC(coin, redeem);
            }
            return coin;
        }
        ...
        private void EstimateScriptSigSize(ICoin coin, ref int witSize, ref int baseSize)
        {
            ...
            if (coin is ScriptCoin scriptCoin)
            {
                ...
                if (p2sh != null)
                {
                    ...
                    {
                        coin = new ScriptCoinSC(coin, scriptCoin.Redeem);
                    }
                }
        ...
        /// </summary>
        /// <returns></returns>
        public TransactionBuilderSC Then()
        {
        ...
        /// </summary>
        /// <returns></returns>
        public TransactionBuilderSC Then(string groupName)
        {
        ...
        /// <returns></returns>
        public TransactionBuilderSC CoverOnly(Money amount)
        {
        ...
        /// <returns></returns>
        public TransactionBuilderSC ContinueToBuild(Transaction transaction)
        {
            if (this._CompletedTransaction != null)
        ...
        /// </summary>
        /// <returns></returns>
        public TransactionBuilderSC CoverTheRest()
        {
            if (this._CompletedTransaction == null)
        ...
        public TransactionBuilderSC AddCoins(Transaction transaction)
        {
        ...
        public TransactionBuilderSC AddKnownRedeems(params Script[] knownRedeems)
        {
